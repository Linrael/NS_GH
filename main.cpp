...
void COMP_RHS(const vector<double> &F, const vector<double> &G, vector<double> &RHS, vector<unsigned char> &FLAG, int imax, int jmax, double delt, double delx, double dely)
{
    for (int i = 1; i <= imax; i++)
        for (int j = 1; j <= jmax; j++)
            if (FLAG[i * (jmax + 2) + j] & C_F)
...
int POISSON(vector<double> &P, vector<double> RHS, vector<unsigned char> &FLAG, int imax, int jmax, double delx, double dely, double eps, int itermax, double omega, double &res)
{
...
void ADAP_UV(vector<double> &U, vector<double> &V, const vector<double> &F, const vector<double> &G, const vector<double> &P, const vector<unsigned char> &FLAG, int imax, int jmax, double delt, double delx, double dely)
{
    // only if both adjacent cells are fluid
    for (int i = 1; i <= imax - 1; i++)
        for (int j = 1; j <= jmax; j++)
